...
def classify_faults(Va, Vb, Vc, m):
    """
    Entradas:
    ...
    for j in range(0, len(Va), m):
        ...
        falla_elegida1, dist_elegida = clasificar_eigvals(Va_actual, Vb_actual, Vc_actual)
        falla_elegida = clasificar_eigvects(Va_actual, Vb_actual, Vc_actual)
    ...
    return falla_elegida_final, dist_elegida_final
...
for j in range(len(enes)):
    ...
    fallas_elegidas_por_buffer = []
    dists_elegidas_por_buffer = []
    
    # Movimiento del buffer
    for j in range(0, len(Va_list), n):

        Va_actual = Va_list[j:j+n]
        Vb_actual = Vb_list[j:j+n]
        Vc_actual = Vc_list[j:j+n]

        # Hacemos la clasificación en cada movimiento del buffer.
        falla_elegida, dist_elegida = classify_faults(Va_actual, Vb_actual, Vc_actual, m)

        # Agregamos el resultado de la clasificación de ese buffer a la lista.
        fallas_elegidas_por_buffer.append(falla_elegida)
        dists_elegidas_por_buffer.append(dist_elegida)

    # La falla y distancia elegida final es la falla/distancia que más retornó el clasificador en cada buffer.
    falla_elegida_final = most_frequent(fallas_elegidas_por_buffer)
    dist_elegida_final = most_frequent(dists_elegidas_por_buffer)
...
df_conf_mat_fallas.to_excel(f'Pruebas estabilidad/Invertidos/sub_ventanas/matrices_conf_fallas/n{n}/m{m}/mat_conf_fallas_{distancia}km.xlsx')
df_accuracy_fallas.to_excel(f'Pruebas estabilidad/Invertidos/sub_ventanas/matrices_conf_fallas/n{n}/m{m}/accuracy_{distancia}km_m{m}.xlsx')
fila_df_conf_mat_dists.to_excel(f'Pruebas estabilidad/Invertidos/sub_ventanas/matrices_conf_dists/n{n}/m{m}/fila{distancia}km.xlsx')